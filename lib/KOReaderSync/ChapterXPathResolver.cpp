...
namespace {
constexpr size_t PATH_RESERVE_DEPTH = 16;
constexpr size_t INLINE_CHILD_COUNTERS = 8;
...
struct ParentState {
  std::array<NameCounter, INLINE_CHILD_COUNTERS> inlineChildren;
  uint8_t inlineChildCount = 0;
  std::vector<NameCounter> overflowChildren;

  int nextIndex(const std::string& name) {
    for (uint8_t i = 0; i < inlineChildCount; i++) {
      if (inlineChildren[i].name == name) {
        inlineChildren[i].count++;
        return inlineChildren[i].count;
      }
    }

    for (auto& child : overflowChildren) {
      if (child.name == name) {
        child.count++;
        return child.count;
      }
    ...
    if (inlineChildCount < INLINE_CHILD_COUNTERS) {
      inlineChildren[inlineChildCount] = {name, 1};
      inlineChildCount++;
      return 1;
    }

    if (overflowChildren.empty()) {
      overflowChildren.reserve(4);
    }
    overflowChildren.push_back({name, 1});
    return 1;
  }
};
 ...
 public:
  ...
  size_t write(const uint8_t* buffer, size_t size) override {
    ...
    if (XML_Parse(parser, reinterpret_cast<const char*>(buffer), static_cast<int>(size), XML_FALSE) != XML_STATUS_OK) {
      LOG_ERR("KOX", "XML parse error: %s", XML_ErrorString(XML_GetErrorCode(parser)));
      parseOk = false;
    }
...
class XPathParagraphResolver final : public Print {
 public:
  explicit XPathParagraphResolver(const int targetParagraph) : targetParagraph(targetParagraph) {
    parentStates.reserve(PATH_RESERVE_DEPTH);
    path.reserve(PATH_RESERVE_DEPTH);
