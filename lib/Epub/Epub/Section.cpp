...
namespace {
// Version 22 adds a per-page paragraph LUT for KOReader XPath progress mapping.
...
constexpr uint32_t HEADER_SIZE = sizeof(uint8_t) + sizeof(int) + sizeof(float) + sizeof(bool) + sizeof(uint8_t) +
                                 ...
                                 sizeof(uint32_t);
constexpr size_t MIN_FREE_HEAP_FOR_SECTION_BUILD = 50 * 1024;
...
bool Section::createSectionFile(const int fontId, const float lineCompression, const bool extraParagraphSpacing,
  ...
  if (startTocIndex >= 0) {
    ...
    tocAnchors.reserve(tocAnchorCount);
    for (int i = startTocIndex; i < epub->getTocItemsCount(); i++) {
...
std::optional<uint16_t> Section::getPageForParagraphIndex(const uint16_t pIndex) const {
  ...
  for (uint16_t i = 0; i < count; i++) {
    uint16_t pageParagraphIndex;
    serialization::readPod(f, pageParagraphIndex);
    if (pageParagraphIndex >= pIndex) {
      resultPage = i;
      break;
    }
  }
